...
@Client.on_message(filters.command("hiraid", prefixes=".") & (filters.me | filters.user(SUDO_USER))
)
async def rr(client: Client, message: Message):
    ...
    await r.edit(f"**Hindiraid Activated On {user.first_name} Successfully ✅**")

@Client.on_message(filters.command("dhiraid", prefixes=".") & (filters.me | filters.user(SUDO_USER))
)
async def drr(client: Client, message: Message):
    ...
    if reply:
        ...
        user = message.text.split(None, 1)[1]
        if not user:
            await r.edit("Provide me username/userid or reply to user for deactivating Hindiraid")
            return
    user = await client.get_users(user)
    ...
    await r.edit(f"**Hindi Raid has Been Removed {user.first_name}, enjoy!**")
