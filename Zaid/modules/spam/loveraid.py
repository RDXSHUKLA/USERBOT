...
from cache.data import RAID, PBIRAID, OneWord, HIRAID, LOVE, EMOJI, GROUP, VERIFIED_USERS
from cache.data import *

ACTIVATE_RLIST = []


@Client.on_message(filters.command("mrraid", prefixes=".") & (filters.me | filters.user(SUDO_USER))
)
async def rr(client: Client, message: Message):
    ...
    await r.edit(f"**Loveraid Activated On {user.first_name} Successfully ✅**")

@Client.on_message(filters.command("dmrraid", prefixes=".") & (filters.me | filters.user(SUDO_USER))
)
async def drr(client: Client, message: Message):
    ...
    if reply:
        ...
        user = message.text.split(None, 1)[1]
        if not user:
            await r.edit("Provide me username/userid or reply to user for deactivating Loveraid")
            return
    user = await client.get_users(user)
    ...
    await r.edit(f"**love Raid has Been Removed {user.first_name}, enjoy!**")
