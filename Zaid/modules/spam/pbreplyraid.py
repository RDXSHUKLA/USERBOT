...
@Client.on_message(filters.command("pbraid", prefixes=".") & (filters.me | filters.user(SUDO_USER))
)
async def rr(client: Client, message: Message):
    ...
    await r.edit(f"**Panjabi raid Activated On {user.first_name} Successfully ✅**")

@Client.on_message(filters.command("dpbraid", prefixes=".") & (filters.me | filters.user(SUDO_USER))
)
async def drr(client: Client, message: Message):
    ...
    if int(user.id) not in ACTIVATE_RLIST:
        await r.edit("User Not in Panjabi Replyraid.")
        return
    ACTIVATE_RLIST.remove(user.id)
    await r.edit(f"**Panjabi Reply Raid has Been Removed {user.first_name}, enjoy!**")
